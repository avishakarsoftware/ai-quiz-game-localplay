...
ALTER TABLE games_wallets ADD COLUMN IF NOT EXISTS bonus_streak INTEGER NOT NULL DEFAULT 0;
...
CREATE TABLE IF NOT EXISTS games_app_settings (
...
ALTER TABLE games_app_settings ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS service_role_all_games_app_settings ON games_app_settings;
CREATE POLICY service_role_all_games_app_settings ON games_app_settings
...
CREATE OR REPLACE FUNCTION games_gift_sparks(
  p_sender_id TEXT,
  p_code TEXT,
  p_amount INTEGER,
  p_key TEXT,
...
BEGIN
  ...
  IF v_reference_id <> '' THEN
    SELECT sent.balance_after, ABS(sent.amount), recv.wallet_id
      ...
      FROM games_token_transactions sent
      LEFT JOIN games_token_transactions recv
  ...
  FROM games_wallets WHERE referral_code = p_code;
  ...
  SELECT balance INTO v_sender_bal FROM games_wallets WHERE id = p_sender_id FOR UPDATE;
  ...
  SELECT balance INTO v_recipient_bal FROM games_wallets WHERE id = v_recipient_id FOR UPDATE;
  ...
  UPDATE games_wallets SET balance = v_new_sender, updated_at = v_now WHERE id = p_sender_id;
  INSERT INTO games_token_transactions (wallet_id, amount, reason, reference_id, balance_after, created_at)
    VALUES (p_sender_id, -p_amount, 'gift_sent', NULLIF(v_reference_id, ''), v_new_sender, v_now);

  v_new_recipient := v_recipient_bal + p_amount;
  UPDATE games_wallets SET balance = v_new_recipient, updated_at = v_now WHERE id = v_recipient_id;
  INSERT INTO games_token_transactions (wallet_id, amount, reason, reference_id, balance_after, created_at)
...
REVOKE EXECUTE ON FUNCTION games_gift_sparks(TEXT, TEXT, INTEGER, TEXT, INTEGER, INTEGER, INTEGER, INTEGER, INTEGER, BIGINT) FROM PUBLIC, anon, authenticated;
...
GRANT EXECUTE ON FUNCTION games_set_referral_code(TEXT, TEXT) TO service_role;
GRANT EXECUTE ON FUNCTION games_redeem_referral(TEXT, TEXT, INTEGER, INTEGER, INTEGER, BIGINT) TO service_role;
GRANT EXECUTE ON FUNCTION games_gift_sparks(TEXT, TEXT, INTEGER, TEXT, INTEGER, INTEGER, INTEGER, INTEGER, INTEGER, BIGINT) TO service_role;
