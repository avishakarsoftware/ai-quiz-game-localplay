...
- During voting, entries are anonymous, identified only by a stable room/round-salted entry id (presented in submission order; not shuffled). The viewer's own vote is echoed back as `your_vote` so the UI can lock and highlight it.
