...
def _state(players=("p1", "p2", "p3", "p4"), config=None):
    return oddq.create_initial_state(list(players), config=config, now=1000.0)


def _answer_all(state, text="thing"):
    for pid in oddq.eligible_voters(state):
        oddq.submit_answer(state, pid, f"{text}-{pid}")
    return state
...
def test_defaults_to_the_curated_deck():
    state = _state()
    assert len(state["config"]["prompt_pairs"]) == len(oddq.DEFAULT_PROMPT_PAIRS)
    assert state["phase"] == oddq.PHASE_ANSWERING
...
def test_malformed_custom_deck_degrades_to_the_curated_one():
    ...
    assert state["config"]["prompt_pairs"] == oddq.DEFAULT_PROMPT_PAIRS
...
def test_minimum_players_enforced():
    ...
    assert oddq.can_start(["p1", "p2"]) is False
    assert oddq.can_start(["p1", "p2", "p3"]) is True
    assert oddq.can_start(["p1", "p1", "p2"]) is False   # dedupes before counting
...
def test_only_the_odd_one_sees_the_minority_prompt():
    ...
    pair = oddq.current_pair(state)

    odd_view = oddq.public_state(state, viewer_id=odd)
    assert odd_view["prompt"] == pair["minority"]
    assert odd_view["you_are_odd"] is True

    for other in [p for p in state["scores"] if p != odd]:
        view = oddq.public_state(state, viewer_id=other)
...
def test_host_view_leaks_neither_prompt_during_play():
    ...
    state = _state()
    pair = oddq.current_pair(state)
    view = oddq.public_state(state, viewer_id=None, host=True)
    assert pair["minority"] not in str(view)
...
def test_answers_are_hidden_until_voting():
    ...
    assert "answers" not in oddq.public_state(state, viewer_id="p2")
    oddq.start_voting(state)
    assert len(oddq.public_state(state, viewer_id="p2")["answers"]) == 4
...
def test_resubmitting_overwrites_instead_of_stacking():
    state = _state()
    oddq.submit_answer(state, "p1", "first")
    oddq.submit_answer(state, "p1", "second")
...
def test_blank_and_unknown_player_answers_are_ignored():
    state = _state()
    oddq.submit_answer(state, "p1", "   ")
    oddq.submit_answer(state, "ghost", "hello")
...
def test_answers_rejected_outside_the_answering_phase():
    state = _answer_all(_state())
    oddq.start_voting(state)
    oddq.submit_answer(state, "p1", "late")
    assert state["answers"]["p1"] != "late"


def test_all_answered_tracks_eligible_voters():
    state = _state()
    assert oddq.all_answered(state) is False
    _answer_all(state)
    assert oddq.all_answered(state) is True


# --- voting ---

def test_self_votes_are_rejected():
    state = oddq.start_voting(_answer_all(_state()))
    oddq.submit_vote(state, "p1", "p1")
    assert "p1" not in state["votes"]


def test_votes_for_unknown_players_are_rejected():
    state = oddq.start_voting(_answer_all(_state()))
    oddq.submit_vote(state, "p1", "ghost")
    assert state["votes"] == {}


def test_revote_overwrites():
    state = oddq.start_voting(_answer_all(_state()))
    oddq.submit_vote(state, "p1", "p2")
    oddq.submit_vote(state, "p1", "p3")
...
def test_catching_the_odd_one_needs_a_strict_majority_not_a_plurality():
    ...
    oddq.submit_vote(state, others[0], odd)
    oddq.submit_vote(state, others[1], odd)             # 2 of 4 — not > half
    oddq.submit_vote(state, others[2], others[0])
    oddq.reveal_round(state)
    assert state["round_result"]["caught"] is False
    assert state["scores"][odd] >= oddq.POINTS_ODD_SURVIVES
...
def test_a_strict_majority_catches_the_odd_one():
    ...
    for voter in others:                               # 3 of 4 > half
        oddq.submit_vote(state, voter, odd)
    oddq.reveal_round(state)
    assert state["round_result"]["caught"] is True
    assert state["scores"][odd] == 0
    for voter in others:
        assert state["scores"][voter] == oddq.POINTS_CORRECT_VOTE
...
def test_odd_one_earns_a_misdirect_bonus_for_piling_on_the_leader():
    ...
    oddq.submit_vote(state, others[0], others[1])
    oddq.submit_vote(state, others[1], others[2])
    oddq.submit_vote(state, others[2], others[1])
    oddq.submit_vote(state, odd, others[1])
    oddq.reveal_round(state)
    assert state["round_result"]["caught"] is False
    assert state["scores"][odd] == oddq.POINTS_ODD_SURVIVES + oddq.POINTS_ODD_MISDIRECT
...
def test_round_resolves_even_with_no_votes_at_all():
    ...
    oddq.reveal_round(state)
    assert state["round_result"]["caught"] is False
    assert state["phase"] == oddq.PHASE_REVEAL
...
def test_final_round_ends_at_podium():
    ...
    oddq.reveal_round(state)
    oddq.next_round(state)
    assert state["phase"] == oddq.PHASE_PODIUM
...
def test_scores_accumulate_across_rounds():
    ...
    assert oddq.standings(state)[0]["score"] >= oddq.POINTS_CORRECT_VOTE
...
def test_late_joiner_cannot_act_in_the_round_already_underway():
    ...
    _answer_all(state)
    oddq.add_player(state, "p4")

    assert "p4" in state["rotation"]          # they DO get a turn as the odd one later
    assert oddq.is_eligible(state, "p4") is False
    assert "p4" not in oddq.eligible_voters(state)

    oddq.submit_answer(state, "p4", "sneaky")
    assert "p4" not in state["answers"]

    oddq.start_voting(state)
    oddq.submit_vote(state, "p4", "p1")
...
def test_late_joiner_becomes_eligible_next_round():
    ...
    oddq.reveal_round(state)
    oddq.next_round(state)
    assert oddq.is_eligible(state, "p4") is True
    oddq.submit_answer(state, "p4", "now allowed")
...
def test_rotation_skips_a_player_who_is_not_eligible_yet():
    ...
    oddq.add_player(state, "p3")              # seated from round 1
    _answer_all(state)
    oddq.start_voting(state)
    oddq.reveal_round(state)
    oddq.next_round(state)
    assert oddq.is_eligible(state, state["odd_player_id"]) is True


def test_adding_an_existing_player_is_a_no_op():
    state = _state(("p1", "p2", "p3"))
    before = list(state["rotation"])
    oddq.add_player(state, "p1")
...
def test_round_resolves_when_the_odd_one_never_answered():
    ...
    state = _state()
    odd = state["odd_player_id"]
    for pid in [p for p in oddq.eligible_voters(state) if p != odd]:
        oddq.submit_answer(state, pid, "answer")
    oddq.start_voting(state)
    for voter in [p for p in state["scores"] if p != odd]:
        oddq.submit_vote(state, voter, odd)
    oddq.reveal_round(state)
