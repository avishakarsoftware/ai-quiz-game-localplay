"""Odd One Out engine (SPEC-GAME-ODD-ONE-OUT).
...
def _state(players=("p1", "p2", "p3", "p4"), config=None):
    return ooo.create_initial_state(list(players), config=config, now=1000.0)


def _answer_all(state, text="thing"):
    for pid in ooo.eligible_voters(state):
        ooo.submit_answer(state, pid, f"{text}-{pid}")
    return state
...
def test_defaults_to_the_curated_deck():
    state = _state()
    assert len(state["config"]["prompt_pairs"]) == len(ooo.DEFAULT_PROMPT_PAIRS)
    assert state["phase"] == ooo.PHASE_ANSWERING
...
def test_malformed_custom_deck_degrades_to_the_curated_one():
    ...
    assert state["config"]["prompt_pairs"] == ooo.DEFAULT_PROMPT_PAIRS
...
def test_minimum_players_enforced():
    ...
    assert ooo.can_start(["p1", "p2"]) is False
    assert ooo.can_start(["p1", "p2", "p3"]) is True
    assert ooo.can_start(["p1", "p1", "p2"]) is False   # dedupes before counting
...
def test_host_view_leaks_neither_prompt_during_play():
    ...
    state = _state()
    pair = ooo.current_pair(state)
    view = ooo.public_state(state, viewer_id=None, host=True)
    assert pair["minority"] not in str(view)
...
def test_answers_are_hidden_until_voting():
    ...
    assert "answers" not in ooo.public_state(state, viewer_id="p2")
    ooo.start_voting(state)
    assert len(ooo.public_state(state, viewer_id="p2")["answers"]) == 4
...
def test_resubmitting_overwrites_instead_of_stacking():
    state = _state()
    ooo.submit_answer(state, "p1", "first")
    ooo.submit_answer(state, "p1", "second")
...
def test_blank_and_unknown_player_answers_are_ignored():
    state = _state()
    ooo.submit_answer(state, "p1", "   ")
    ooo.submit_answer(state, "ghost", "hello")
...
def test_answers_rejected_outside_the_answering_phase():
    state = _answer_all(_state())
    ooo.start_voting(state)
    ooo.submit_answer(state, "p1", "late")
    assert state["answers"]["p1"] != "late"


def test_all_answered_tracks_eligible_voters():
    state = _state()
    assert ooo.all_answered(state) is False
    _answer_all(state)
    assert ooo.all_answered(state) is True


# --- voting ---

def test_self_votes_are_rejected():
    state = ooo.start_voting(_answer_all(_state()))
    ooo.submit_vote(state, "p1", "p1")
    assert "p1" not in state["votes"]


def test_votes_for_unknown_players_are_rejected():
    state = ooo.start_voting(_answer_all(_state()))
    ooo.submit_vote(state, "p1", "ghost")
    assert state["votes"] == {}


def test_revote_overwrites():
    state = ooo.start_voting(_answer_all(_state()))
    ooo.submit_vote(state, "p1", "p2")
    ooo.submit_vote(state, "p1", "p3")
...
def test_catching_the_odd_one_needs_a_strict_majority_not_a_plurality():
    ...
    ooo.submit_vote(state, others[0], odd)
    ooo.submit_vote(state, others[1], odd)             # 2 of 4 — not > half
    ooo.submit_vote(state, others[2], others[0])
    ooo.reveal_round(state)
    assert state["round_result"]["caught"] is False
    assert state["scores"][odd] >= ooo.POINTS_ODD_SURVIVES
...
def test_a_strict_majority_catches_the_odd_one():
    ...
    for voter in others:                               # 3 of 4 > half
        ooo.submit_vote(state, voter, odd)
    ooo.reveal_round(state)
    assert state["round_result"]["caught"] is True
    assert state["scores"][odd] == 0
    for voter in others:
        assert state["scores"][voter] == ooo.POINTS_CORRECT_VOTE
...
def test_odd_one_earns_a_misdirect_bonus_for_piling_on_the_leader():
    ...
    ooo.submit_vote(state, others[0], others[1])
    ooo.submit_vote(state, others[1], others[2])
    ooo.submit_vote(state, others[2], others[1])
    ooo.submit_vote(state, odd, others[1])
    ooo.reveal_round(state)
    assert state["round_result"]["caught"] is False
    assert state["scores"][odd] == ooo.POINTS_ODD_SURVIVES + ooo.POINTS_ODD_MISDIRECT
...
def test_round_resolves_even_with_no_votes_at_all():
    ...
    ooo.reveal_round(state)
    assert state["round_result"]["caught"] is False
    assert state["phase"] == ooo.PHASE_REVEAL
...
def test_final_round_ends_at_podium():
    ...
    ooo.reveal_round(state)
    ooo.next_round(state)
    assert state["phase"] == ooo.PHASE_PODIUM
...
def test_scores_accumulate_across_rounds():
    ...
    assert ooo.standings(state)[0]["score"] >= ooo.POINTS_CORRECT_VOTE
...
def test_late_joiner_cannot_act_in_the_round_already_underway():
    ...
    _answer_all(state)
    ooo.add_player(state, "p4")

    assert "p4" in state["rotation"]          # they DO get a turn as the odd one later
    assert ooo.is_eligible(state, "p4") is False
    assert "p4" not in ooo.eligible_voters(state)

    ooo.submit_answer(state, "p4", "sneaky")
    assert "p4" not in state["answers"]

    ooo.start_voting(state)
    ooo.submit_vote(state, "p4", "p1")
...
def test_late_joiner_becomes_eligible_next_round():
    ...
    ooo.reveal_round(state)
    ooo.next_round(state)
    assert ooo.is_eligible(state, "p4") is True
    ooo.submit_answer(state, "p4", "now allowed")
...
def test_rotation_skips_a_player_who_is_not_eligible_yet():
    ...
    ooo.add_player(state, "p3")              # seated from round 1
    _answer_all(state)
    ooo.start_voting(state)
    ooo.reveal_round(state)
    ooo.next_round(state)
    assert ooo.is_eligible(state, state["odd_player_id"]) is True


def test_adding_an_existing_player_is_a_no_op():
    state = _state(("p1", "p2", "p3"))
    before = list(state["rotation"])
    ooo.add_player(state, "p1")
...
def test_round_resolves_when_the_odd_one_never_answered():
    ...
    state = _state()
    odd = state["odd_player_id"]
    for pid in [p for p in ooo.eligible_voters(state) if p != odd]:
        ooo.submit_answer(state, pid, "answer")
    ooo.start_voting(state)
    for voter in [p for p in state["scores"] if p != odd]:
        ooo.submit_vote(state, voter, odd)
    ooo.reveal_round(state)
