...
from odd_question_engine import (
    ...
    PHASE_PODIUM as ODDQ_PHASE_PODIUM,
    PHASE_REVEAL as ODDQ_PHASE_REVEAL,
    can_start as oddq_can_start,
    create_initial_state as oddq_create_initial_state,
    next_round as oddq_next_round,
    public_state as oddq_public_state,
    reveal_round as oddq_reveal_round,
    standings as oddq_standings,
    start_voting as oddq_start_voting,
    submit_answer as oddq_submit_answer,
    submit_vote as oddq_submit_vote,
    validate_config as validate_odd_question_config,
)
...
class SocketManager:
    ...
    async def handle_message(self, room: Room, client_id: str, message: dict, is_organizer: bool):
        ...
        if is_organizer:
                ...
                async with room.lock:
                    ...
                    if room.game_type == "wmlt":
                        ...
                        if player_count < config.MIN_ODD_QUESTION_PLAYERS:
                            await self._send_to_client(room, client_id, {
                                "type": "ERROR",
                                "message": f"Odd Question needs at least {config.MIN_ODD_QUESTION_PLAYERS} players to start",
                            })
                            return
            ...
            elif msg_type == "ODDQ_START_VOTING" and room.game_type == "odd_question":
                await self._odd_question_start_voting(room)
            elif msg_type == "ACRO_START_VOTING" and room.game_type == "acronym":
                await self._acronym_start_voting(room)

            elif msg_type == "ODDQ_REVEAL" and room.game_type == "odd_question":
                await self._simple_social_reveal(room)

            elif msg_type == "ODDQ_NEXT_ROUND" and room.game_type == "odd_question":
                await self._simple_social_next_round(room)
            ...
            elif msg_type in ("WYR_VOTE", "NHIE_ANSWER", "WORD_SUBMIT", "ACRO_SUBMIT", "ACRO_VOTE", "ODDQ_ANSWER", "ODDQ_VOTE") and room.game_type in SIMPLE_SOCIAL_GAME_TYPES:
    ...
    def _simple_social_state_attr(self, game_type: str) -> str:
        return {
            ...
            "acronym": "acro_state",
            "odd_question": "odd_question_state",
        }[game_type]
    ...
    def _simple_social_public_state(self, room: Room, viewer_id: str | None = None) -> dict:
        ...
        if room.game_type == "odd_question":
            # viewer_id is the nickname; the engine resolves which prompt this viewer may see.
            return oddq_public_state(room.odd_question_state, viewer_id)
        return {}
    ...
    def _simple_social_standings(self, room: Room) -> list[dict]:
        ...
        if room.game_type == "odd_question" and room.odd_question_state:
            return oddq_standings(room.odd_question_state)
    ...
    def _start_simple_social_game(self, room: Room):
        ...
        if room.game_type == "would_you_rather":
            ...
            room.odd_question_config = validate_odd_question_config(room.quiz)
            room.odd_question_state = oddq_create_initial_state(nicknames, room.odd_question_config, now=time.time())
            room.state = room.odd_question_state["phase"]
        elif room.game_type == "acronym":
        ...
        try:
            async with room.lock:
                if room.game_type == "would_you_rather":
                    ...
                    if message.get("type") == "ODDQ_VOTE":
                        room.odd_question_state = oddq_submit_vote(room.odd_question_state, nickname, str(message.get("accused") or ""))
                        room.answer_log.append({"kind": "accusation", "nickname": nickname})
                    else:
                        room.odd_question_state = oddq_submit_answer(room.odd_question_state, nickname, str(message.get("text") or ""))
        ...
        try:
            async with room.lock:
                if room.game_type == "would_you_rather":
                    ...
                    room.odd_question_state = oddq_reveal_round(room.odd_question_state)
        ...
        try:
            async with room.lock:
                if room.game_type == "would_you_rather":
                    ...
                    room.odd_question_state = oddq_next_round(room.odd_question_state)
                self._sync_simple_social_phase_to_room(room)
        except ValueError:
            return
        await self._broadcast_simple_social_sync(room)
        state = getattr(room, self._simple_social_state_attr(room.game_type), {})
        if state.get("phase") in {WYR_PHASE_PODIUM, NHIE_PHASE_PODIUM, WORD_PHASE_PODIUM, ACRO_PHASE_PODIUM, ODDQ_PHASE_PODIUM}:
            await self._simple_social_complete_game(room)
    ...
    async def _odd_question_start_voting(self, room: Room):
        """Host closes answering and reveals all answers for accusation."""
        try:
            async with room.lock:
                room.odd_question_state = oddq_start_voting(room.odd_question_state)
                self._sync_simple_social_phase_to_room(room)
        except ValueError:
            return
        await self._broadcast_simple_social_sync(room)
