...
| Spark gifting (`GIFTING_ENABLED` + `gift_sparks` RPC) | ❌ off | ❌ off | Built on master 2026-07-21 (commit `49ad0782`), gated OFF. Migration `sql/migrations/20260721T010000_gifting{,_gamma}.sql` authored but NOT applied; `GIFTING_ENABLED` unset. Works on SQLite locally. To activate: apply migration → set `GIFTING_ENABLED=true` → recreate container. |
