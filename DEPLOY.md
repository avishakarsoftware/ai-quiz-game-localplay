...
- Pending next gamma deploy/smoke: host-app lobby QR rendering from Revelry-provided `guest_join_url`, including the nested `external_context.guest_join_url` / `display.guest_join_url` launch-token shape used by Revelry.
...
- Pending next gamma deploy/smoke: host-app completed-game action returns to the Revelry Games surface instead of the standalone LocalPlay setup loop; spectator aliases `/spectate`, `/spectate/{room_code}`, `/tv`, and `/tv/{room_code}` connect through the shared spectator page and show clear websocket error states.
