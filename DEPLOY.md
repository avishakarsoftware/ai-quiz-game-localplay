...
|---|---|---|---|
| Deployed code (backend+SPA) | `2820ee2f` (2026-08-04) | `b0c1fc03` (2026-07-26) | **Gamma redeployed 2026-08-04** with Claude's cleanup batch (Tailwind color-class revival, pre-sparks premium-layer removal, test/typecheck coverage hardening, isolated pytest DBs) plus the TV room socket lifecycle fix (`/tv` organizer reconnect + replacement-room cleanup). Deploy script health passed; live probes returned `{"status":"healthy"}` and `/config/public` showed Gemini Flash Lite, gifting, achievements, referrals, and ads disabled. Earlier same-day gamma deep verification at `3a27bc51` was `regression.py --target gamma --deep` **189/189 PASS**. **PROD IS STILL `b0c1fc03`** — so the newer gamma-only hardening and bug fixes are not yet production runtime. |
