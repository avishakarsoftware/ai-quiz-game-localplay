...
|---|---|---|---|
| Deployed code (backend+SPA) | `d769b509`+ (2026-07-27) | `b0c1fc03` (2026-07-26) | **Gamma is now AHEAD of prod** — first gamma-only soak since the growth deploy. Gamma carries Odd Question, the 3 occasion bingos, hosting stats, dynamic share images, lobby seat cleanup + occupancy-qualified TTL, and the admin config override layer. Verified live on gamma: 37 catalog games (all 4 new ids present), `/stats` + `/achievements` return real JSON (content-type checked — a missing route returns 200 text/html via the SPA catch-all and looks healthy), share image → image/png, rc→401, ad-reward→403. Gamma `/webhook/stripe`→503 is CORRECT and pre-existing: gamma has never had Stripe keys (verified on the VM); only prod does, which is why prod returns 400. | Growth-features deploy (see 2026-07-26 entry below). All three web surfaces current: gamma + prod backend SPA, and IONOS `index-CXMMX1TT.js` (byte-identical to the local build; `config.json` byte-identical to live, so no flag clobber). Post-deploy verified on prod: `/tokens/ad-reward`→**403** (farm hole closed), stripe→400, rc→401, health→200, `/catalog`→33 games. |
...
| **Pass-and-play + Impostor** (SPEC-PASS-AND-PLAY) | ✅ live + full round verified | ❌ not deployed | First pass-and-play game: ONE shared phone, host types the seats, guests need no device. Deployed to gamma 2026-07-28; a complete round was walked over a real WebSocket against live gamma (**22/22 checks**) — catalog entry present and flagged `interaction=pass_and_play`, room starts with ZERO connected players (the invariant a `connected_player_count()` gate would break), roles delivered only during the gated reveal phase and withheld once the phone is face-up, no rate-limit ERROR across a full round, comeback rule fired, podium leaderboard carries seat names. **No migration and no feature flag** — seats and state are in-memory per room, so this needed neither. |
| Hosting stats + game badges (`game_results` table) | ✅ applied 2026-07-27 | ❌ not applied | Built 2026-07-27 (SPEC-GAME-STATS). Migration `sql/migrations/20260727T000000_game_results{,_gamma}.sql` authored but NOT applied. **No feature flag** — `GET /stats` returns `available:false` and the `StatsSection` hides itself until the table exists, so the code is safe to deploy ahead of the migration. Applying it turns on hosting stats AND the four game-completion badges (`first_game`/`ten_games`/`big_party`/`explorer`) wherever `ACHIEVEMENTS_ENABLED` is already true. Works on SQLite locally with no migration. |
| Remote-config overrides (`app_settings` table) | ✅ applied 2026-07-27 | ❌ not applied | Built 2026-07-27 (SPEC-REMOTE-CONFIG §admin). Migration `sql/migrations/20260727T010000_app_settings{,_gamma}.sql` authored but NOT applied. No flag — `get_overrides()` treats a missing table as "no overrides", so the code is safe to deploy ahead of the migration; the admin endpoints just can't persist until it exists. Also requires `ADMIN_API_KEY` to be set on the environment (503 otherwise). Once live, an AI model swap is a `PUT /admin/config` instead of a frontend deploy. |
...
| **Google Play** | n/a | ✅ **LIVE — v3.1.3 (9), 177 countries, 0 installs** | Verified in the Play Console 2026-07-28: Production track Active, latest release 3.1.3; both Jul 20 submissions show **Published**. It went live the SAME DAY (managed publishing was off), never sat in a multi-day review — an earlier note claiming "in review since 07-20" was stale and caused a wrong status answer. Internal testing also carries 9 (3.1.3). **0 installs** = no install base yet, so shipping updates is low-risk; the unproven license-tester purchase smoke is the real gap. |
