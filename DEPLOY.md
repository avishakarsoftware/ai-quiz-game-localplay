...
|---|---|---|---|
| Deployed code (backend+SPA) | `f87e8da3` (2026-08-04, evening) | `b0c1fc03` (2026-07-26) | **Gamma redeployed 2026-08-04 evening** with the full REVIEW-2026-08 fix batch: XFF rate-limit fix, cap-overflow purchase guard (409 + modal greying), spawn() task safety, signup-bonus IP allowance + per-wallet LLM quota, auditable admin grants, Sentry wiring (dormant, no DSN set), **first-party grace (PARTY_GRACE_HOURS=6 default — a new host's first evening of rooms is free)**, and the Python 3.14 container. This supersedes the earlier same-day `2820ee2f` gamma deploy (cleanup batch + TV socket lifecycle fix, deep verification 189/189 at `3a27bc51`). **Maiden run of the new deploy machinery worked**: pre-flight booted the image on :8074 before touching the live container, rollback point captured, gamma now capped at 768 MB/1.0 CPU (prod uncapped — verified via docker inspect). Live verification: `/health` OK, fresh wallet got signup+day-1 bonus (30 ⚡) with `party_grace: available` + `max_balance` in the payload, gamma smoke 2/2, **all-games live suite 75/76** (photo_clue camera waiver). **PROD IS STILL `b0c1fc03`** — none of today's fixes (including the S1 rate-limit fix and purchase-cap guard) are production runtime yet. |
