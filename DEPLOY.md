...
|---|---|---|---|
| Deployed code (backend+SPA) | `3a27bc51` (2026-08-04) | `b0c1fc03` (2026-07-26) | **Gamma deployed 2026-08-04** with the whole batch: pass-and-play + Impostor, Odd Question, three occasion bingos, hosting stats + game badges, dynamic share images, the TV launcher (`/tv`) with live room hosting, the "1 phone" pill + filter, **and two bug fixes** — the `/purchases/restore` spark-minting hole and the forgeable LLM prompt fence. Verified live: `regression.py --target gamma --deep` **189/189 PASS**, restore returns `restored:false` with balance held at 30 across 3 taps, served bundle carries every new surface, and the expired `launch_2026` promo is gone from `/config/public`. **PROD IS STILL `b0c1fc03`** — so both bug fixes remain live-broken in production, including the money leak. |
