...
- LocalPlay commit `5828ffd` is the next gamma deploy candidate for Revelry party-content scoping and callback hardening. It isolates standalone custom-quiz drafts from Revelry party drafts, includes safe callback actor metadata for hub-started games, validates return/guest URLs by parsed origin, emits `content.updated` with `previous_content_id` when used content versions, and warns if `REVELRY_CALLBACK_SECRET` diverges from canonical `REVELRY_INTEGRATION_SECRET`.
