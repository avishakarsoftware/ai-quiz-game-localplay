...
|---|---|---|---|
| Deployed code (backend+SPA) | `a3c770ab` (2026-07-28) | `a3c770ab` (2026-07-28) | Pass-and-play/Impostor deploy. All three web surfaces current: gamma + prod backend SPA, and IONOS `index-B3WxRXFl.js` (**byte-identical** to the local build via `cmp`; `config.json` byte-identical to live so no feature-flag clobber). Prod post-deploy verified: 38 games with `impostor`/`odd_question`/all three occasion bingos present, `/stats` + `/achievements` returning JSON (not the SPA catch-all), stripe→400, rc→401, ad-reward→403, IONOS + /privacy→200, and live room creation succeeded for `impostor` and `odd_question`. DB backed up: `revelry_20260728_215801.db` (prod). |
