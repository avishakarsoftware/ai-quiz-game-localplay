...
| Native IAP (`REVENUECAT_WEBHOOK_SECRET`) | ✅ configured | ✅ live + verified | RevenueCat prod webhook "Revelry Games Prod" (`gamesapi.revelryapp.me/webhook/revenuecat`, Bearer, Both env, all apps/events, HMAC off) created + Active 2026-07-14; **test event verified 200 authenticated** in prod logs (dedup + `games_mark_webhook_processed` ran). **Store submissions (iOS build + IAPs, Play promotion) still pending** before real customer purchases. |
