...
export const LOCAL_GAME_RULES: Partial<Record<GameType, GameRules>> = {
    ...
    housie: rules('Housie Rules', 'Mark called numbers and claim prizes when patterns complete.', ['Complete prize patterns on your ticket.'], ['The caller draws numbers.', 'Players mark tickets.', 'Claim when a prize pattern is complete.'], ['Valid claims award prizes.', 'The configured final prize ends the game.'], { player_count: { min: 2, recommended: '4-50' } }),
    bingo: rules('Bingo Rules', 'Mark called items and claim configured patterns.', ['Complete a line, corners, blackout, or another selected pattern.'], ['The host calls items.', 'Players mark matching board cells.', 'Players claim complete patterns.'], ['Valid claims win prizes.', 'The host can continue to the final prize.'], { player_count: { min: 2, recommended: '4-50' } }),
    baby_bingo: rules('Baby Bingo Rules', 'Play Bingo with baby-shower gifts, moments, and tiny surprises.', ['Mark baby-shower items as they are called.'], ['The host calls items.', 'Players mark matching cells.', 'Claim completed patterns.'], ['Valid claims win prizes.', 'The host decides when to stop.'], { player_count: { min: 2, recommended: '4-50' } }),
    impostor: rules('Impostor Rules',
        'Everyone knows the secret word except one player. One phone, passed around the table.',
        [
            'Know the word? Prove it with your clue without handing it to the impostor.',
            'Are the impostor? Blend in, survive the vote — or name the word if caught.',
        ],
        [
            'The host types everyone in — only ONE phone is needed, so guests install nothing.',
            'Pass the phone around. Each player privately sees the word; one sees they are the impostor.',
            'Phone goes face-up. In turn, each player says ONE word aloud related to the secret.',
            'After the clue rounds, the table discusses and votes on the shared screen.',
            'A caught impostor gets one guess at the word — guess right and they still win.',
        ],
        [
            'Impostor caught: everyone else scores 2.',
            'Impostor survives, or the table ties: impostor scores 3.',
            'Caught but names the word: impostor scores 3 anyway.',
            'Conviction needs a strict majority — a tied table lets the impostor go.',
        ],
        { player_count: { min: 3, recommended: '4-8' } }),
    odd_question: rules('Odd Question Rules', 'Everyone answers the same question — except one player, who got a different one.', ['Spot the player whose question was different.', 'If your question was different, blend in and survive the vote.'], ['Everyone gets a question on their phone. One is secretly different.', 'Everyone submits a short answer.', 'All answers are revealed and everyone votes for who had it.', 'The odd one out and both questions are revealed.'], ['Guess correctly who had the odd question: 2 points.', 'Survive without a majority naming you: 3 points.', 'The odd question rotates so everyone gets a turn.'], { player_count: { min: 3, recommended: '4-10' } }),
    wedding_bingo: rules('Wedding Bingo Rules', 'Play Bingo with the reception moments everyone is already watching for.', ['Mark wedding moments as they happen or as the host calls them.'], ['The host calls items.', 'Players mark matching cells.', 'Claim completed patterns.'], ['Valid claims win prizes.', 'The host can keep calling until the final prize.'], { player_count: { min: 2, recommended: '6-60' } }),
    holiday_bingo: rules('Holiday Bingo Rules', 'Play Bingo with the family-gathering moments that happen every year.', ['Mark holiday moments as they happen or as the host calls them.'], ['The host calls items.', 'Players mark matching cells.', 'Claim completed patterns.'], ['Valid claims win prizes.', 'The host can keep calling until the final prize.'], { player_count: { min: 2, recommended: '4-30' } }),
    road_trip_bingo: rules('Road Trip Bingo Rules', 'Play Bingo with what you actually see and hear on a long drive.', ['Mark road-trip sights and moments as they happen.'], ['The host calls items, or players self-mark on the honour system.', 'Players mark matching cells.', 'Claim completed patterns.'], ['Valid claims win prizes.', 'The driver should not be playing.'], { player_count: { min: 2, recommended: '2-8' } }),
    ...
    two_truths: rules('Two Truths and a Lie Rules', 'Submit two truths and one lie. Everyone guesses the lie.', ['Fool the room and spot lies from other players.'], ['Each player submits three statements.', 'One player is revealed at a time.', 'Everyone votes for the lie.'], ['Guessers score for finding lies.', 'Authors score when voters are fooled.'], { player_count: { min: 3, recommended: '4-20' } }),
    story_chain: rules('Story Chain Rules', 'Each player adds one sentence to build a shared story.', ['Create a funny or surprising story together.'], ['Players take turns adding a sentence.', 'The final story is revealed at the end.'], ['Usually scored by completion or host choice.', 'The group wins if the story lands.'], { player_count: { min: 3, recommended: '4-12' } }),
    common_ground: rules('Common Ground Rules', 'Teams find things they all have in common.', ['Discover shared facts that are specific or surprising.'], ['Players are assigned to teams.', 'Teams discuss and submit answers.', 'The room reveals and may vote.'], ['Teams score for valid submissions and votes.', 'Highest team score wins.'], { player_count: { min: 4, recommended: '6-30' } }),
    ...
    who_am_i: rules('Who Am I? Rules', 'Use clues to guess the mystery answer.', ['Guess the answer using as few clues as possible.'], ['A clue appears each step.', 'Players submit guesses.', 'More clues appear until solved or time runs out.'], ['Earlier correct guesses score more.', 'Highest score wins.'], { player_count: { min: 2, recommended: '3-20' } }),
...
export function rulesForGame(gameType: GameType, catalog?: CatalogGameWithRules[]): GameRules | null {
    ...
    if (catalogRules) return catalogRules;
    const localRules = LOCAL_GAME_RULES[gameType];
    if (!localRules) return null;
    return {
        ...localRules,
        player_count: {
            ...(localRules.player_count || {}),
            min: getMinPlayers(gameType),
        },
    };
}
