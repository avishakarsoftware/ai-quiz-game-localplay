...
Status: LocalPlay implementation complete and enabled for gamma and production. Gamma Supabase migration, authoring capability opt-ins, and strict `requires_prepared_content_for_checkin` were applied on 2026-07-09. Revelry gamma is deployed and verified (setup/save/preview/no-duplicate-cards). On 2026-07-14, LocalPlay's repeatable gamma pre-prod harness passed the strict lifecycle: setup-required 409, deterministic saved Party Quests content, saved-preview Host/Player/TV tabs, prepared session start, organizer lobby rendering **Party Quests Lobby** instead of the default quiz lobby, first-player auto-start, late join board assignment, cancellation, and workspace active-session clearing. Production DDL was then expanded to accept `content_type='party_quests'`, verified with a disposable insert/delete smoke, and the production `party_quests` policy row was flipped to `status=live` with `can_create_content`, `can_edit_content`, `supports_ai_generation`, `embedded_authoring_supported`, and `requires_prepared_content_for_checkin` enabled. The authoritative per-environment state is the *Environment status ledger* in `DEPLOY.md`. This section supersedes the earlier Party Quests quick-start/default-content contract below. The existing quick-start behavior remains a compatibility path for explicit/manual starts, but the production check-in target experience is now configured prepared content.
...
Saved Party Quests content requires `party_quests` in the `generated_content.content_type` constraint in:

- local SQLite creation/migration code;
- `sql/games-gamma-schema.sql`;
- `sql/games-schema.sql`;
- the live gamma Supabase constraint;
- the live production Supabase constraint.

The database migration is additive. No existing content rows change. Gamma was applied on 2026-07-09. Production was applied on 2026-07-14 with a single targeted migration that dropped/re-added only `games_generated_content_content_type_check`, then verified via disposable `party_quests` insert/delete before the production policy row was flipped. Record future environment migrations in `DEPLOY.md`.
...
5. Production rollout: production DDL and strict policy are live as of 2026-07-14. Existing active sessions are never rewritten or cancelled by rollout. If Revelry has already shipped the setup/return flow, no extra LocalPlay gate remains; otherwise Revelry should continue to use the structured setup-required response to guide the host into configuration.
...
As of 2026-07-14, the LocalPlay-owned gamma harness covers setup-required, saved preview, prepared start, organizer lobby, first-player auto-start, late join, cancellation, and workspace clearing. Before the production strict flip, still run Revelry's app-side check-in setting/arming flow against gamma and confirm the actual Revelry Games tab and check-in surfaces pass the same lifecycle using real party settings.
...
Current LocalPlay gamma status: gamma has passed direct smoke for health, config, catalog, session creation, launch-token generation, status polling, tokenless player launch redirect, party workspace, party hub link/resolve, host-app-managed billing wallet behavior, LocalPlay-hosted quiz authoring, start from saved `localplay_content_id`, WebSocket organizer/player play-through, completion, result polling, host-app join-link QR/copy behavior, completed-game return to Revelry Games, spectator alias SPA routing, terminal organizer/player/spectator host-app error egress, and lowercase typed TV URL normalization to uppercase websocket room codes. The live gamma catalog includes Housie, Musical Chairs, and strict prepared Party Quests for `host_app = revelry`; `games_gamma_generated_content_content_type_check` allows `housie`, `chit_pull`, and `party_quests`. The pre-prod Revelry matrix (`PREPROD_REVELRY=1 ... npm run test:e2e:preprod-revelry`) verifies the embedded hub catalog/search UI and then starts every launchable game returned by the live Revelry catalog, with deterministic save/start fixtures for Quiz, WMLT, Drawing, Housie, Random Chit, and Party Quests. It must fail when a newly exposed launchable game lacks a test fixture. Before each rollout or manual test pass, verify the deployed gamma container against the current repo HEAD because this spec intentionally does not act as the sole source of truth for deployed commit tracking.

Current LocalPlay production status is tracked in `DEPLOY.md`. As of 2026-07-14, production LocalPlay has the Revelry bridge code, `REVELRY_INTEGRATION_SECRET`, production callback URL, custom media uploads, hosted Musical Chairs music loops, AI image generation disabled for public standalone media, production `games_host_app_catalog_flags`, and strict prepared Party Quests policy enabled for Revelry. Production `games_generated_content_content_type_check` allows `quiz`, `mlt`, `drawing`, `housie`, `chit_pull`, and `party_quests`; host-app exposure for Housie/Random Chit authoring remains policy-controlled and should be smoke-tested separately before broad use. Revelry production is expected to use `GAMES_ENGINE_URL=https://gamesapi.revelryapp.me` and the matching `LOCALPLAY_INTEGRATION_SECRET`.
...
- Lobby disconnects are soft: a dropped player WebSocket in `LOBBY` preserves the seat as `offline`/`reconnecting` for `LOBBY_RECONNECT_GRACE_SECONDS` (90 minutes by default) unless the host removes it or the grace ages out. Pre-start minimum-player checks count only connected/start-ready sockets, while the lobby roster still shows preserved seats so hosts understand who may need to wake/reopen the game. LOBBY room expiry and room snapshot restore also use the lobby grace as their effective TTL so a quiet pre-start party lobby is not removed by the generic room TTL.
