...
**Status: BUILT 2026-08-04 (REVIEW-2026-08 P1, option "first-party grace"). Not yet deployed.**
...
- State lives in the ledger: each free room writes a zero-amount `grace_room` row;
  ...
  record_grace_room` on both backends (export-guard enforced).
...
`backend/tests/test_party_grace.py` (9): free evening well past the old paywall point, window
expiry, room cap, broke-host-post-cap 402, veteran ineligibility, kill switch, status
lifecycle/read-only-ness. Frontend: `PartyGraceBanner.test.tsx` (5) incl. old-backend
fail-silent. conftest pins `PARTY_GRACE_HOURS=0` so the other 1,400+ tests keep pre-grace
expectations. Visual baselines updated (banner on catalog); all-games passes with grace active.
