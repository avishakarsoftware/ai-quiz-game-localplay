...
- Every game must expose concise rules before play. Hosts should be able to open rules from the game picker before selecting/creating the game. Players should be able to open the same rules from the join/lobby screen before the host starts. Rules should be mobile-friendly and structured around objective, player count, setup, round flow, scoring/winning, privacy/hidden-info notes, and any physical-world requirements. Prefer catalog metadata or a shared rules registry over duplicated component copy so standalone LocalPlay, Revelry hub, player lobby, and spectator/help surfaces remain consistent.
